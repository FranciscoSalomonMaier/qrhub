...
export function QrCodeLivePreview({ payload, canPreview, saved, onCreate, creating, onReset }) {
    ...
    <aside className="lg:sticky lg:top-24 lg:self-start">
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg shadow-slate-200/60">
        <div className="border-b border-slate-100 px-6 py-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 font-black text-slate-950">{payload.name || 'Seu QR Code'}</h2>
            </div>
        ...
        <div className="p-5 sm:p-6">
          <div className="grid aspect-square place-items-center overflow-hidden rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-200">
            {
              preview.isFetching ? 
              ...
              <img className="h-full w-full object-contain" src={source} alt={`Prévia de ${payload.name || 'QR Code'}`} /> 
              : 
          ...
          
          {
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <button onClick={() => download.mutate({ uuid: saved.id, format: 'png' })} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 text-sm font-bold hover:bg-slate-50">
                <ArrowDownTrayIcon className="size-4" /> PNG
              </button>
              <button onClick={() => download.mutate({ uuid: saved.id, format: 'svg' })} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-300 text-sm font-bold hover:bg-slate-50">
                <ArrowDownTrayIcon className="size-4" /> SVG
              </button>
