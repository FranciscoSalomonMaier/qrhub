...
function hasPreviewContent(values) {
  ...
  if (values.type === 'url') { try { const url = new URL(content.url); return ['http:', 'https:'].includes(url.protocol); } catch { return false; } }
  if (values.type === 'text') return Boolean(content.text?.trim());
  if (values.type === 'email') return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(content.email ?? '');
