...
const base = z.object({ name: z.string().min(1, 'Informe um nome.').max(120), type: z.enum(['url', 'text', 'email', 'phone', 'whatsapp', 'wifi']), foreground_color: z.string().regex(/^#[0-9a-f]{6}$/i), background_color: z.string().regex(/^#[0-9a-f]{6}$/i), size: z.coerce.number().int().min(128).max(2048), error_correction_level: z.enum(['L', 'M', 'Q', 'H']), is_active: z.boolean() });
...
export const defaultQrCode = { name: '', type: 'url', content: { url: '' }, foreground_color: '#000000', background_color: '#FFFFFF', size: 512, error_correction_level: 'M', is_active: true };
